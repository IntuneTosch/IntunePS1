...
if (-not (Test-Path -LiteralPath $DirectoryToCreate)) {
    ...
    "Tijdelijke Bestandslocatie aangemaakt '$DirectoryToCreate'."
...
-#>
    ...
    Process {
        ...
        if ($AppId -ne "") {
            ...
            Write-Host "Verbonden met Intune Tennant $TenantId using app-based authentication (Azure AD authentication not supported)"
        }
        else {
            if ($version -eq 2) {
                write-host "Version 2 module detected"
            ...
            Write-Host "Verbonden met Intune Tennant $($graph.TenantId)"
        }
    }
}
...
try {
    ...
    Write-Host "Verwijderen oude caching voltooid. Disconnect-MgGraph wordt uitgevoert."
} catch {
    # If directory doesn't exist or another error occurs, Disconnect-MgGraph will be executed
    if ($_ -match "cannot find path") {
        Write-Host "Oude Sessie niet gevonden. Het script gaat door"
    } else {
...
Write-Host "Verbinding maken met MS Graph"  -ForegroundColor DarkYellow
Start-Sleep -Seconds 2
