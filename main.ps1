...
Write-host "Version 3.3" -ForegroundColor DarkRed
...
Write-Host $([system.text.encoding]::UTF8.GetString([system.convert]::FromBase64String($welcomeScreen)))          
Write-Host "1. Modules zijn succesvol geïmporteerd."  -ForegroundColor Green
""
...
if (-not (Test-Path -LiteralPath $DirectoryToCreate)) {
    ...
    Write-Host "2. Tijdelijke bestandslocatie is aangemaakt.'$DirectoryToCreate'." -ForegroundColor Green
    ""
}
else {
    Write-Host "2. Tijdelijke bestandslocatie bestaat al. Het script wordt voortgezet." -ForegroundColor Green
    ""
    Start-Sleep -Seconds 1.5
}
...
if (Test-Path $path) {
    Write-Host "3. De willekeurige map is succesvol aangemaakt." -ForegroundColor Green
    ""
    Start-Sleep -Seconds 1.5
}
else {
    Write-Host $([system.text.encoding]::UTF8.GetString([system.convert]::FromBase64String($welcomeScreen)))    
    Write-Error "3. De willekeurige map kon niet worden aangemaakt." -ErrorAction Stop
    ""
...
function GrabProfiles() {
    ...
    # Creating a custom Out-GridView that is always on top and smaller
    $gridViewWindow = $profiles | Out-GridView -PassThru -Title "Select a profile"

    # Access the underlying GridView window to adjust size and set it to be always on top
    $gridViewWindowType = $gridViewWindow.PSObject.Properties["Parent"].Value.GetType()
    $gridViewWindowHandle = $gridViewWindowType.GetField("handle", [System.Reflection.BindingFlags]::NonPublic -bor [System.Reflection.BindingFlags]::Instance).GetValue($gridViewWindow)

    # Set the size of the window (e.g., 400x300)
    $gridViewWindowWidth = 400
    $gridViewWindowHeight = 300
    [System.Windows.Forms.SendKeys]::SendWait("^{ESC}")  # To ensure the grid is focused
    $gridViewWindowHandle.SetBounds(0, 0, $gridViewWindowWidth, $gridViewWindowHeight)

    # Keep the GridView window always on top
    Add-Type -TypeDefinition @"
    using System;
    using System.Runtime.InteropServices;
    public class WindowHelper {
        [DllImport("user32.dll")]
        public static extern int SetWindowPos(IntPtr hWnd, IntPtr hWndInsertAfter, int x, int y, int width, int height, uint uFlags);
    }
"@

    $topMostFlag = 0x00000008  # Flag to keep window on top
    $HWND_TOPMOST = [IntPtr]::Zero
    [WindowHelper]::SetWindowPos($gridViewWindowHandle, $HWND_TOPMOST, 0, 0, $gridViewWindowWidth, $gridViewWindowHeight, $topMostFlag)

    return $gridViewWindow.id
}
...
Write-Host "4. Functies zijn succesvol geïmporteerd." -ForegroundColor Green
""
...
try {
    ...
    Write-Host "5. Het verwijderen van de oude cache is voltooid. Disconnect-MgGraph wordt nu uitgevoerd." -Foregroundcolor Green
    ""
    Start-Sleep -Seconds 1.5
    ...
    if ($_ -match "cannot find path") {
        Write-Host "5. Er is geen actieve inlogsessie gevonden. Het script wordt voortgezet." -Foregroundcolor Green
        ""
    } else {
...
if ($clientid -and $clientsecret -and $tenant) {
    ...
    Write-Host "7 Verbinding geslaagd, Profielen ophalen. Kies het juiste profiel" -ForegroundColor Green
    ""
    Start-Sleep -Seconds 3
###############################################################################################################
######                                              Execution JSON                                       ######
