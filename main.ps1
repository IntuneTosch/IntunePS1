...
if (-not (Test-Path -LiteralPath $DirectoryToCreate)) {
    ...
    "Directory already existed, Continuing."
}
...
function Select-Drive($volumeLabel) {
    ...
    Write-Host "Fout: Er zijn meerdere drives met de VolumeLabel 'WINPE'. Gebruik 1 USB."
    } elseif ($Drives.Count -eq 1) {
        return ($Drives[0] + ":\")
    } else {
        Write-Host "Er zijn geen drives gevonden met de VolumeLabel '$volumeLabel'."
        return $null
    }
}
